...
so_t *so_cria(cpu_t *cpu, mem_t *mem, mmu_t *mmu, console_t *console, relogio_t *relogio)
{
...
static void so_escalona_prio(so_t *self)
{
  ...
  for(int i=0; i<MAX_PROCESSOS; i++) {
    ...
    console_printf(self->console, "Pid: %d, Prio %f", self->tabela_processos[i].pid, self->tabela_processos[i].prio);
  }
}
...
static void calcula_prio(so_t *self, int pid) {
  ...
  if(i != -1) {
    ...
    console_printf(self->console, "CALCULANDO PRIOOOOOOOOO: %f !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", self->tabela_processos[i].prio + (t_exec / (self->quantum_counter * INTERVALO_INTERRUPCAO)));
  }
}
